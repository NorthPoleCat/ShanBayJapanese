...
def main():
    ...
    p.add_argument("--input", default=str(ROOT/"data/intermediate/refined_words.json"))
    p.add_argument("--output", default=str(ROOT/"data/intermediate/zh_senses.csv"))
    args = p.parse_args()

    rows = json.loads(Path(args.input).read_text(encoding="utf-8"))
    with Path(args.output).open("w", encoding="utf-8-sig", newline="") as f:
        fields = [
            "source_seq","word","reading","jlpt_level","jmdict_entry_id",
            "sense_index","pos_group","pos_text","meaning_en","meaning_zh",
            "review_status","translation_source","translation_note"
        ]
    ...
    print(f"Sense translation template -> {args.output}")
